...
function PerfilPage() {
  ...
  const peopleQuery = useQuery({
    ...
    queryFn: () => getPeople({ pageSize: 3, sort: "az" }),
  });
    ...
    <div className="mx-auto max-w-[1400px] px-5 py-14 md:px-10 md:py-20">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-6 border-b border-border pb-10 sm:flex sm:justify-between">
        <div className="flex min-w-0 items-center gap-5">
          <img
            ...
            className="h-16 w-16 shrink-0 border border-border object-cover grayscale sm:h-20 sm:w-20"
          />
      ...
      <dl className="mt-10 grid gap-6 sm:grid-cols-3">
        {[
          ...
          { label: "Fotografias enviadas", value: "18" },
        ].map((item) => (
